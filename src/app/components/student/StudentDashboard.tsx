...
import { Flame, BookOpen, ClipboardList, TrendingUp, CheckCircle2, ChevronRight } from 'lucide-react';
...
export function StudentDashboard() {
  ...
  const { theme } = useTheme(); // not used directly, but kept for consistency

  const pending = ASSIGNMENTS.filter((a) => a.status === 'pending');
  const avgGrade = Math.round(COURSES.reduce((s, c) => s + c.gradeValue, 0) / COURSES.length);
  ...
  return (
    <div className="space-y-7 max-w-6xl">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1.5">
            You have <span className="text-foreground font-semibold">{pending.length} assignments</span> due this week.
          </p>
        </div>
        {/* Streak */}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Enrolled Courses', value: COURSES.length, icon: BookOpen, color: '#C4582A', sub: '2 with next class today' },
          { label: 'Pending Work', value: pending.length, icon: ClipboardList, color: '#D4A230', sub: 'Nearest due in 6 days' },
          { label: 'Average Grade', value: `${avgGrade}%`, icon: TrendingUp, color: '#4A8A5C', sub: '+3% from last semester' },
          {
            label: 'Modules Done',
          ...
          <div key={s.label} className="bg-card border border-border rounded-2xl p-5 group hover:border-primary/25 transition-all">
          ...
          <div className="space-y-3">
            {COURSES.map((c) => (
              <button
                ...
                className="w-full bg-card border border-border rounded-2xl p-4 text-left hover:border-primary/25 transition-all group"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                      ...
                      <span className="text-sm font-bold flex-shrink-0" style={{ fontFamily: mono, color: c.color }}>
                        {c.grade}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-[10px] text-muted-foreground flex-shrink-0" style={{ fontFamily: mono }}>
                        {c.progress}%
                      </span>
                    </div>
                  </div>
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="space-y-2.5">
              {pending.map((a) => {
                ...
                return (
                  <div key={a.id} className="flex items-start gap-3 bg-card border border-border rounded-xl p-3.5">
                    <div className="w-1 h-full min-h-[2.5rem] rounded-full flex-shrink-0" style={{ background: course?.color }} />
                    <div className="flex-1 min-w-0">
          ...
          <div className="bg-card border border-border rounded-2xl p-4">
            ...
            <ResponsiveContainer width="100%" height={80}>
              <LineChart data={GRADE_HISTORY}>
                <Line type="monotone" dataKey="cs302" stroke="#4A8A5C" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="cs201" stroke="#C4582A" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          ...
          <div className="bg-card border border-primary/20 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-semibold text-primary uppercase tracking-widest">Pinned</span>
            </div>
            <p className="text-xs font-semibold text-foreground mb-1">{ANNOUNCEMENTS[0].title}</p>
            <p className="text-[10px] text-muted-foreground leading-relaxed">{ANNOUNCEMENTS[0].body.slice(0, 80)}…</p>
